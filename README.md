...
Actions are packets of infromation that describe events that have occurred in the UI

Reducers are pure functions that take in the state tree and an action as arguments.

The store is where all your data lives.

The store is a single source of truth that is one giant state object that updates the UI.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state is a state that is available to every component. While a component state is only available within that specific component.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

 Redux-thunk is middleware that handles asynchronous requests in Redux. It allows us to turn action creators into async functions.
